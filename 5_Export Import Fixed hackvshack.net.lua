...
local config_system do
    ...
    menu.elements.config.actions.save:set_callback(function ()
        ...
        for _, v in pairs(funny) do
            local success, export_data = pcall(function()
                return tabs[v]:export()
            end)
            if success and export_data then
                data[v] = export_data
            end
        end
        ...
        if menu.elements.config.name:get() ~= '' then
            ...
            file.write(string.format("%s\\%s", file.MASTER_PATH, helpers.sanitize(menu.elements.config.name:get())), encoded)
        end
    end)
    ...
    menu.elements.tabs.actions.import:set_callback(function ()
        ...
        if not success or not converted_data then
            return
        end

        local selected_indices = menu.elements.tabs.tab_list:get()
        
        for k, v in pairs(converted_data) do
            local elements = tabs[k]
            if elements then
                for i = 1, #funny do
                    if selected_indices[i] and funny[selected_indices[i]] == k then
                        elements:import(v)
                    end
                end
            end
    ...
    menu.elements.tabs.actions.export:set_callback(function ()
        ...
        for i = 1, #funny do
            local selected_tab = funny[selected_indices[i]]
            if selected_tab then
                data[selected_tab] = tabs[selected_tab]:export()
            end
        end
...
ui.sidebar('\a{Link Active}Export/Import', 'gear')